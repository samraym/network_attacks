...
The brute-force attack targets the FTP server (`10.12.0.40`) in the DMZ because we know from network canning that the port 21 is open. The attack script systematically attempts to log in using a known weak username (`victim`) and a wordlist of common passwords (`10k-most-common.txt`).
...
To mitigate ARP spoofing attacks, we implemented a monitoring script that passively observes incoming ARP replies on the victim host (e.g. `ws3`). It detects suspicious changes in IP-to-MAC associations and flushes the corresponding ARP entry if a spoof is suspected.
...
To protect the DMZ servers from SSH brute-force attacks originating from the Internet, we implemented a defense script on the r2 gateway. The script uses iptables to monitor and limit the number of SSH login attempts from a single IP address. If more than 5 attempts are detected within 60 seconds from the same IP, further SSH traffic from that source is dropped.
...
To mitigate the reflected DNS DoS attack, we implemented a protection script on r1, the router between the DMZ and the workstations. This script uses iptables to drop unsolicited UDP packets from the DNS server (10.12.0.20) to the workstations (10.1.0.0/24) on port 5353, which is exploited in the attack.
